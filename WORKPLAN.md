...
- [x] Tests: ten - five on the reader (a folder round-tripping the run's facts and the stages' costs, the older of two retrospectives reachable beside the newer, a folder written before the fields loading without inventing them, a folder that is not one, and a multi-line goal that does not end the block), three on the surface (the window opening and the header following the reports, a folder holding nothing leaving what is on screen alone, and an unrecorded run labelled honestly), and two on the tree (the folder asking for itself by name, and every other folder still just expanding).
