...
## 78. A retrospective is reopened by double-clicking the folder it was written to

- [x] **Estimated time:** 0.5d

Ten retrospectives sit under `.glasscoder/retrospectives`, each a folder of three reports and a tickable list, and the only one the application could show was the newest of whichever run had just finished. The reports opened one file at a time in the viewer as Markdown - which is the prose without the reading: no stage colouring, no expanders, and no checklist, because the checklist is `recommendations.json` beside them and nothing rendered it. The work the retrospective did was on disk and out of reach.

- [x] **`IRetrospectiveReviewer.LoadFrom(directory)` beside `Load(runId)`.** Load is given a run and finds its newest folder, so every retrospective after the first of a run was unreachable by construction - and re-taking one is the normal thing to do. `Load` now delegates to it, so there is one reader. A folder that holds no retrospective comes back null: recognising one and reading one are the same act, which is one rule rather than two kept in step.
- [x] **The folder carries what the header shows.** `goal`, `stopReason`, `steps` and `durationMs` join `runId` in the stage front matter, and `model`, `costUsd` and `takenAt` are now read back rather than dropped - a reopened retrospective used to report `$0.0000` and `0 s` over reports that had cost real money. Every field is optional on both sides, so the ten folders already written still load; what they cannot say reads as unknown and never as zero, which is why `RunLabel` drops the tail rather than printing `·  · 0 steps`.
- [x] **The goal is flattened to one line.** It is free text an operator typed and the block is `key: value` lines, so a single newline in it would close the front matter early and take every field under it - including the run id that finds it.
- [x] **The tree opens it, and only it.** The double-click command already declined directories, which is what leaves folders expanding; it now accepts exactly one kind - a directory sitting directly inside the configured retrospectives directory. A path-shape test, not a disk read: it is re-queried every time WPF asks whether the command can run, and whether the folder really holds a retrospective is answered once, by reading it.
- [x] **The pane says which folder, and stops.** `RetrospectiveOpened` is an event because the workspace pane cannot reach the surface and should not learn how: it is bound beside the surfaces rather than inside one, and its only outward seam is `IDesktopShell` - the operating system, which does not choose surfaces. The shell holds both halves and joins them.
- [x] **The folder holding them starts closed.** It gains a subfolder every time the button is pressed, and ten of them open at startup put a wall of timestamps above the source the pane exists to show. Its parent still opens, so the closed folder is there to click, and each retrospective inside it keeps the ordinary default - the exception is one folder, not a branch.
- [x] **It arrives as though it had just finished.** The proposals window opens, which rehydration at startup deliberately does not do - the difference is that somebody pointed at this one. The ticks start where a fresh checklist starts, high priority marked and nothing else: which items were accepted last time lives in the work order that was written from them, and restoring ticks that were never recorded would be inventing them.
- [x] Tests: eleven - five on the reader (a folder round-tripping the run's facts and the stages' costs, the older of two retrospectives reachable beside the newer, a folder written before the fields loading without inventing them, a folder that is not one, and a multi-line goal that does not end the block), three on the surface (the window opening and the header following the reports, a folder holding nothing leaving what is on screen alone, and an unrecorded run labelled honestly), and three on the tree (the folder asking for itself by name, every other folder still just expanding, and the retrospectives folder starting closed while its parent does not).

Acceptance: double-clicking `.glasscoder/retrospectives/20260817-091459` brings up the Retrospective surface showing that run's three reports in their own colours with its nine recommendations tickable and the proposals window open, and double-clicking any other folder still expands it. Depends on task 67.
